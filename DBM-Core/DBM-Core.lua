...
do
	...
	function argsMT.__index:GetSrcCreatureID()
		return tonumber(self.sourceGUID:sub(8, 12), 16) or 0
	end
	
	function argsMT.__index:GetDestCreatureID()
		return tonumber(self.destGUID:sub(8, 12), 16) or 0
	end
	...
	function DBM:UNIT_AURA(uId, spellName, _, spellGUID, spellId)
		local correctSpellId = tonumber(select(5, strsplit("-", spellGUID)), 10)
		self:Debug("UNIT_AURA fired: "..UnitName(uId).."'s "..spellName.."("..correctSpellId..")", 3)
		--local correctSpellId = tonumber(select(5, strsplit("-", spellGUID)), 10)
		self:Debug("UNIT_AURA fired: "..UnitName(uId).."'s "..spellName.."("..spellId..")", 3)
 	end
	
	function DBM:UNIT_SPELLCAST_START(uId, spellName, _, spellGUID, spellId)
		local correctSpellId = tonumber(select(5, strsplit("-", spellGUID)), 10)
		self:Debug("UNIT_SPELLCAST_START fired: "..UnitName(uId).."'s "..spellName.."("..correctSpellId..")", 3)
		--local correctSpellId = tonumber(select(5, strsplit("-", spellGUID)), 10)
		self:Debug("UNIT_SPELLCAST_START fired: "..UnitName(uId).."'s "..spellName.."("..spellId..")", 3)
 	end
	...
	local function buildTargetList()
		...
		for i = 0, math.max(GetNumRaidMembers(), GetNumPartyMembers()) do
			...
			if guid and (bit.band(guid:sub(1, 5), 0x00F) == 3 or bit.band(guid:sub(1, 5), 0x00F) == 5) then
				local cId = tonumber(guid:sub(8, 12), 16)
				targetList[cId] = id
			end
		end
	end
...
function DBM:StartCombat(mod, delay, synced)
	if not checkEntry(inCombat, mod) then
		...
		-- Send Start message to guild
	--	SendChatMessage(DBM_CORE_COMBAT_STARTED_GUILD:format(mod.combatInfo.name), "GUILD")
	...
	if removeEntry(inCombat, mod) then
		...
		if wipe then
			...
			if not lastTime then
				...
				-- Send Kill message to guild
			--	SendChatMessage(DBM_CORE_BOSS_DOWN_GUILD:format(mod.combatInfo.name, strFromTime(thisTime)), "GUILD")
...
function DBM:UNIT_DIED(args)
	if bit.band(args.destGUID:sub(1, 5), 0x00F) == 3 or bit.band(args.destGUID:sub(1, 5), 0x00F) == 5  then
		self:OnMobKill(tonumber(args.destGUID:sub(8, 12), 16))
	end
end
